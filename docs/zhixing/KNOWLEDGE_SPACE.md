...
只有当用户明确要求查阅项目资料，或当前任务明显依赖已绑定的索引资料时，AI 才应先搜索、再精确读取相关片段。没有命中时必须明确说明，但不得阻断普通回答，也不得把模型常识伪装成项目事实。
...
- 未初始化，或 `indexedDocumentCount = 0` 的空知识空间，不向模型注册任何 `knowledge_*` 工具。
- Workspace 系统提示不宣传知识空间，也不要求模型在普通对话前必先搜索。
- 只有“已初始化且至少有一份可检索文档”时才注册知识工具；原始二进制文件未归一化成可检索文档时，不算可用知识。
- 降级只改变 AI 工具可见性和调用策略；现有数据、目录、导入、索引、UI 入口和恢复能力全部保留。
...
4. 助手只绑定 Workspace、未安装 RootFS 时，若存在可检索文档，仍可按需调用 status/search/read。
5. 导入工具需要批准；只读工具默认无需批准。
6. 路径逃逸、读取非知识路径和超限读取被拒绝。
7. 空库不注册知识工具，也不向提示词注入知识检索要求。
