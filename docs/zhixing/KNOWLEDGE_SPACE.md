...
- `99_系统/提示词/`：领域提示词；其中 `助手/<助手 ID>.md` 是知行按助手固定定位的用户提示词；
...
- 此能力按助手可选，默认继续使用 DataStore 中的应用配置；启用时先幂等创建
  `vault/99_系统/提示词/助手/<助手 ID>.md`，已有文件绝不被初始化覆盖。
- 只托管用户可编辑的助手提示词。内部系统、安全、工具、记忆、Workspace 和运行时指令不写入 vault，
  也不能通过此能力读取或修改。
- 新对话在首条用户消息持久化时冻结一次生效内容、来源路径与版本。之后即使文件被修改，当前对话也不变；
  fork 继承原快照。升级前已有用户消息的旧对话首次继续时冻结当时的应用配置，不自动采用 vault 新内容。
- 文件缺失时回退应用配置；文件存在但内容为空表示显式空提示词。超限、不可读或版本冲突必须报错，不能
  伪装成文件缺失后静默回退。
- `assistant_user_prompt_read` 与 `assistant_user_prompt_edit` 仅操作当前助手的固定路径，不接受模型提供的
  任意 path，不依赖 RootFS。写入使用 SHA-256 版本比较和同目录原子替换，冲突时要求重新读取。
- 助手提示词页面可以切换来源并编辑 vault 文件；克隆助手默认恢复为应用配置，避免新 ID 误继承旧文件。
...
11. 启用知识库提示词时保留已有文件；缺失才从应用配置创建，空文件不回退。
12. 当前对话在提示词文件修改后保持原快照，新对话采用新版本；专用工具不能访问固定文件以外的路径。
