...
3. “工作”首页是用户整理过的工作集，不是完整目录转储：只展示置顶项目与少量最近仍可工作的项目；
   完整项目目录、仅含归档任务的项目和已隐藏项目进入“管理全部”。连接凭据、机器与协议诊断属于设置，
   不占用工作界面。
4. 项目页保留完整历史对话，不以活跃时间窗口作为可见性或保留条件；未归档与已归档任务分开展示，
   用户可把高价值对话设为重点。项目隐藏、置顶和对话重点状态只保存在手机本地，不删除开发机目录或
   Codex 原始历史。
...
- [x] Issue #18 阶段 5：关键通知——仅在任务结束或出现待审批/待决策时通知（普通命令执行不打扰，
      ...
      允许/拒绝）留待后续。
- [x] 0.2.1 目录整理：Agent 标记开发机目录是否仍存在；手机继续缓存完整历史，但工作主页最多展示
      8 个最近可工作项目（置顶项目不受上限影响），仅归档或目录已删除的项目自动收进管理页；
      支持项目置顶/隐藏与重点对话，相关偏好不会被目录快照覆盖。
- [ ] 阶段 D 及以后。
