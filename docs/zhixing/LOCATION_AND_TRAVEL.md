...
| 工具 | 能力 | 逐次审批 |
| --- | --- | --- |
| `get_current_location` | 获取一次前台位置和结构化地址 | 否 |
| `search_nearby_places` | 以一次前台位置为中心检索附近地点 | 否 |
| `open_navigation` | 打开高德地图 App；未安装时打开高德 Web 导航 | 否 |
...
- 只申请 Android 前台粗略/精确位置权限，不申请后台位置权限。用户只授予大致位置时仍允许尝试定位，并在
  结果中如实返回精度。
- 普通聊天直接执行已启用的位置工具；关闭工具组后，助手不再获得上述工具。隐私同意和 Android 前台位置权限
  仍是强制边界。
...
- 三项工具不进入逐次审批 UI；未取得隐私同意或系统权限时不调用 SDK 或外部 Intent。
