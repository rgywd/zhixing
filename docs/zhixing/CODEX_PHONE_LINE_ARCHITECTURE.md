...
- 运行时与模型在创建时选择；思考深度可随后续消息调整并从下一轮生效；执行权限固定为无需逐次确认的完全访问。
...
- 新建时选择 Runner、仓库、运行时、模型、初始思考深度；第一条消息就是普通聊天输入。已有会话可为下一条消息
  调整思考深度，但不能改写正在运行的当前回合。
...
模型和运行时是 Work 会话的不可变快照；思考深度是会话当前默认值，可随用户消息原子更新并应用于该消息触发的
下一次 CLI START/RESUME。已经运行的子进程不接受中途改写。Runner 不读取任何桌面客户端的 SQLite、缓存或任务目录。
