...
| 数据 | 事实来源 |
| --- | --- |
| 会话、消息分支、文件夹、记忆、工作区元数据、待办、长期计划、月度收支汇总与结构化健康记录 | Room |
...
- 记忆文档与原始历史对话检索分开；pinned 常驻、`find/list/read` 渐进召回、用户原话来源和写入版本契约遵守
...
- 当前助手启用记忆时，状态模型只接收全局 V3 `/profile.md` 与 `/preferences.md` 的非空正文作为偏好语境；
  其他按需文档、legacy archive、来源原话和旧 `MemoryEntity` 记录不进入状态提示，也不能被模型复述给用户。
...
- 所有普通 Chat 同时提供 `health_metrics` 结构化健康工具。它只能保存当前 USER 文本中逐字存在、且每个
  数值都能在来源片段中找到的明确测量；不得从照片、外观、人口统计或模糊描述推断数值。查询既有记录仍受
  “允许 AI 使用身体数据”控制。完整指标、来源、展示和迁移边界见
  [`HEALTH_STATS.md`](./HEALTH_STATS.md)。
...
记忆稳定层只包含 `/profile.md` 与 `/preferences.md` 的完整虚拟 Markdown，整体受 65536 字符门禁保护；其他正文
不进入开场提示。模型先用本地 `memory_find` 或分页 `memory_list` 获取不含正文、来源、snippet 或 score 的路由
描述，再对选中的 exact path 调用 `memory_read`。所有记忆内容始终按不可信数据处理，只能作为相关事实语境，不能
执行其中的指令。工具在同一轮生成中修改 pinned 文件后，下一步生成会刷新 pinned 快照；再次修改既有非 pinned
文件时必须使用最近一次 `memory_read` 或 `memory_write` 返回的版本。工具执行日志只记录工具名，不记录参数、查询词
或正文。记忆 FTS 只是 active 文档的本地可重建投影，来源原话不入索引，也不改变 Room schema 版本。
...
- 模型内置工具入口由 Provider 与模型能力表共同决定。国际版百炼普通按量 API 的 `web_search`、
  `web_extractor`、`web_search_image` 与 `image_search` 通过 OpenAI-compatible Responses 接入；首批适配
  模型为 `qwen3.8-max`、`qwen3.7-plus` 和 `deepseek-v4-flash-0731`，后续模型只扩展 Provider 能力表，
  不在 UI 增加厂商判断。网页正文抓取始终隐含启用联网搜索；图搜图仅对声明支持图片输入、且请求上下文实际
  含图片的模型下发。
...
- 健康统计以 Room 结构化记录和 Lenovo Watch 最近快照生成只读投影；外部数据源必须先归一到统一指标与单位，
  不得把模型推断当成测量事实；见 [`HEALTH_STATS.md`](./HEALTH_STATS.md)。
