...
- Chat 模型的默认上下文窗口是 500,000 Token，并允许在模型设置中为较小窗口单独下调。普通 Chat 将冻结的
  对话用户提示词放在系统消息稳定前缀；记忆、上下文检查点、运行时上下文和可能随消息变化的工具补充位于其后，
  不得反向污染稳定前缀的 Provider 缓存。
- 自动历史压缩不得成为正常回复前的可见步骤。达到模型窗口 60% 后，只在一次回复完成后后台准备 checkpoint；
  达到 78% 后原子激活已准备结果；仅在剩余窗口不足 64,000 Token（小窗口按 25% 预留）且没有可用结果时，
  才允许无 UI 状态的同步兜底。Room 和 UI 始终保留原始消息，编辑或切换分支会清除失效 checkpoint。
...
普通 Chat 只对工具定义显式开启去重、且执行模式为只读并发的调用做 Run 内去重。签名由工具名和规范化后的
结构化参数组成；搜索工具的用户可见 `purpose` 不参与签名。相同调用并发出现时，后续调用等待首个调用：首个
成功后不再执行，并向模型返回只含原调用 ID 的紧凑 `duplicate_skipped` 结果，不复制原始工具输出；首个失败则
允许后续调用重试。不同工具、参数确有差异的调用、未显式开启去重的只读工具，以及所有写入或外部副作用工具
始终正常执行。注册表只存在于当前 `generateText` Run，开始下一次 Run 时清空。
...
- Work 是知行内的独立会话域，由开发机 Runner 启动 Codex 或 Claude Code；它不读取桌面历史、不复用普通
  Provider 生成链路。Chat 提供 Work 入口与待回答提醒；后台跟踪、通知和历史深链均进入应用内 Work。
  应用边界与迁移契约见
