...
- ASK 高优先级提醒与临期提醒附带「采用推荐方案」通知动作：点击直接以各题推荐选项提交回答（视为用户回答，
  `source=user`）；任一题缺少 `recommendedOptionIds` 时不提供该动作。超时落定后的迟到提交由 Core 幂等忽略。
- Android 把活跃会话镜像为动态桌面快捷方式（`WAITING_FOR_USER` 优先，其次 `RUNNING`、`QUEUED`，同状态按
  更新时间倒序，数量按启动器容量裁剪），并提供固定的 Work 首页静态快捷方式；活跃会话清空或跟踪服务停止时
  移除全部动态快捷方式。快捷方式只携带会话 ID，不包含 token、路径或消息正文。
