...
const REASONING_LEVELS: ReasoningLevel[] = ["off", "low", "medium", "high", "xhigh", "max"];
...
export function ReasoningPickerButton({ disabled = false, className }: ReasoningPickerButtonProps) {
  ...
  const storedLevel = currentAssistant?.reasoningLevel;
  const currentLevel: ReasoningLevel =
    storedLevel === "auto"
      ? "medium"
      : REASONING_LEVELS.includes(storedLevel as ReasoningLevel)
        ? (storedLevel as ReasoningLevel)
        : "medium";
