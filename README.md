...
- 支持火山引擎方舟通用 API。
- 绑定可用 Rootfs 的助手可在用户确认后通过受控 `gh issue` 工具访问本仓库 Issue；GitHub Token 仍由 App 安全保存。
...
公开主仓与正式下载入口是 [GitHub 知行](https://github.com/rgywd/zhixing)。`v*` 标签触发 GitHub Actions
完成测试和签名构建，并将正式 Release、对应源码归档和应用内更新清单 `latest.json` 发布到
[GitHub Releases](https://github.com/rgywd/zhixing/releases)。仓库需要配置：
...
上通过后再 push。GitHub PR 只运行单 Job 轻量策略检查；正式标签仍执行完整云端测试、签名与公开发布。
详细边界见 [CI 与本地构建流水线](docs/zhixing/CI_PIPELINE.md)。

## 许可

正式安装包与每个版本的对应源码归档发布在
[GitHub Releases](https://github.com/rgywd/zhixing/releases)。许可条款见 [LICENSE](LICENSE)，
