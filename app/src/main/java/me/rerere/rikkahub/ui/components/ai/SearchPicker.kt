...
private fun SearchPicker(
    ...
    // 模型是否支持内置搜索
    val supportsBuiltInSearch = model != null && BuiltInToolSupport.supports(
        providerSetting = model.findProvider(settings.providers),
        model = model,
        tool = BuiltInTools.Search,
    )
                            ...
                            settings.copy(
                                providers = settings.providers.map { providerSetting ->
                                    providerSetting.editModel(
                                        model.copy(
                                            tools = BuiltInToolSupport.updateSelection(
                                                model.tools,
                                                BuiltInTools.Search,
                                                checked,
                                            )
